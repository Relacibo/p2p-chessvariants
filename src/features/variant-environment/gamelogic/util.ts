...
export function getPositionsOfPiece(
  boardState: BoardState | BoardState[],
  piece: Piece,
  gameIndex?: number
): BoardCoords[] {
...
export function isSingularBoardState(
  boardState: BoardState | BoardState[]
): boardState is BoardState {
  return !boardState[0] || !Array.isArray(boardState[0][0]);
}

export function isSingularReservePile(
...
export function castRay(
  ...
  coords: BoardCoords,
  direction: Direction
): { empty: BoardCoords[]; hit?: { coords: BoardCoords; tile: TileData } } {
  let empty = [];
