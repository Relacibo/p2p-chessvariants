    ...
    engine
        ...
        .register_indexer_get(
            |p: &mut Player, key: &str| -> Dynamic {
                match key {
                    "id" => Dynamic::from(p.id),
                    "name" => Dynamic::from(p.name.clone()),
                    "home_board" => Dynamic::from(p.home_board),
                    "team" => Dynamic::from(p.team),
                    "data" => p.data.clone().unwrap_or(Dynamic::UNIT),
                    _ => Dynamic::UNIT,
                }
            },
        )
