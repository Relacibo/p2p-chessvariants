...
pub fn standard_start_position() -> BoardState {
    ...
    for (col, make_piece) in back_rank.into_iter().enumerate() {
        board.set_piece(&BoardCoords::new_board_0(0, col as i32), Some(make_piece("black".into())));
        board.set_piece(&BoardCoords::new_board_0(7, col as i32), Some(make_piece("white".into())));
    }

    for col in 0..8 {
        board.set_piece(&BoardCoords::new_board_0(1, col), Some(Piece::rhai_make_pawn("black".into())));
        board.set_piece(&BoardCoords::new_board_0(6, col), Some(Piece::rhai_make_pawn("white".into())));
    }
...
mod tests {
    ...
    fn test_standard_start_position() {
        ...
        assert_eq!(board.get_piece(&BoardCoords::new_board_0(0, 4)).unwrap().piece_type_name(), "king");
        assert_eq!(board.get_piece(&BoardCoords::new_board_0(7, 4)).unwrap().color_name(), "white");
